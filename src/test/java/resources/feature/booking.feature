Feature: Testing different requests on the restful booker api
  ...
  Scenario: Check if a new booking can be created on the restful booker api
    When  I send a POST request to booking endpoint with the information firstname "Fran" lastname "Rookledge" totalprice "999" depositpaid "True" booking dates additionalneeds "Dinner"
    Then  I must get back a valid status code "200"
  ...
  Scenario: Check if the restful booker api can be updated by users by searching booking Id
    When  I send a PUT request to bookingId endpoint with updated information firstname "Fran" lastname "Rookledge" totalprice "999" depositpaid "True" booking dates additionalneeds "Dinner"
    Then  I must get back a valid status code "200"
